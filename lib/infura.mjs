  ...
  try {
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      logError(
        `Attempt ${
          retryCount + 1
        }/${MAX_RETRIES} failed for tx ${txHash}. Retrying in ${delay}ms...`
      );
    ...
    logError(
      `Failed to fetch receipt for tx ${txHash} after ${MAX_RETRIES} attempts: ${error.message}`
    );
    throw error;
  }
}

export const getBlockTransactions = async (blockNumber, finalized = false, retries = 0) => {
  // TODO: quick and dirty retry solution for handling 429 errors; but it should 
  // be consolidated with exponential backoff logic found elsewhere
  const MAX_RETRIES = 10;
  ...
  try {
    ...
    // TODO: consolidate with request retry logic found elsewhere
    if (error.response?.data?.error?.code === 429 && retries < MAX_RETRIES) {
      const delay = BASE_DELAY * 2 ** retries;
      logError(`429 Too Many Requests while fetching block ${blockNumber}. Retrying in ${delay / 1000} seconds...`);
      await new Promise(resolve => setTimeout(resolve, delay));
      return getBlockTransactions(blockNumber, finalized, retries + 1);
    }
