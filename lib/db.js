...
export async function getInputMessage(from) {
  const query = `
...
`;
  ...
  if (result.rows.length > 0) {
    return result.rows[0].input_message;
  } else {
    // Logic for randomized messages could be implemented here
    return "Here's my donation!";
  }
}
