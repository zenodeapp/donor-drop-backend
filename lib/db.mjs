...
export async function markBlockAsScraped(blockNumber, transactionsFound = 0) {
  try {
    ...
    throw new Error("Failed to mark block as scraped.");
  }
}
...
export async function isBlockScraped(blockNumber) {
  try {
    ...
    throw new Error("Failed to check if block is scraped.");
  }
}
...
export async function getLastScrapedBlock() {
  try {
    ...
    console.error("Error getting last scraped block:", error);
    throw new Error("Failed to get last scraped block.");
  }
}
...
export async function getBlockScrapingStats(blockNumber) {
  ...
  return result.rows[0];
    ...
    console.error(`Error fetching scraping stats for block ${blockNumber}:`, error);
    throw new Error(`Failed to retrieve scraping stats for block ${blockNumber}.`);
  }
}
...
export async function getRecentScrapingActivity(limit = 10) {
  try {
    ...
    throw new Error("Failed to retrieve recent scraping activity.");
  }
}
...
pool.on('error', async (err, _) => {
  ...
  console.error(`[${timestamp}] Unexpected error on pool connection:`);
